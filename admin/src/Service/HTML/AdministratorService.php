...
namespace Joomla\Component\Apimanagers\Administrator\Service\HTML;
...
class AdministratorService
{
	 ...
	 *
	 * @param   int  $apimanagerid  The item id to search associations
	 *
	 * @return  string  The language HTML
	 *
	 * @throws  \Exception  Throws a 500 Exception on Database failure
	 */
	public function association($apimanagerid)
	{
		// Defaults
		$html = '';

		// Get the associations
		if ($associations = Associations::getAssociations('com_apimanagers', '#__apimanagers', 'com_apimanagers.item', $apimanagerid))
		{
			...
			// Get the associated apimanager items
			$db = Factory::getDbo();
			...
			$query
				...
				->from($db->quoteName('#__apimanagers', 'c'))
				...
				->bind(':id', $apimanagerid, ParameterType::INTEGER);
			$db->setQuery($query);
			...
			if ($items)
			{
				...
				foreach ($items as &$item)
				{
					...
					{
						$text    = $item->lang_code;
						$url     = Route::_('index.php?option=com_apimanagers&task=apimanager.edit&id=' . (int) $item->id);
						$tooltip = '<strong>' . htmlspecialchars($item->language_title, ENT_QUOTES, 'UTF-8') . '</strong><br>'
							. htmlspecialchars($item->title, ENT_QUOTES, 'UTF-8') . '<br>' . Text::sprintf('JCATEGORY_SPRINTF', $item->category_title);
						$classes = 'badge bg-secondary';

						$item->link = '<a href="' . $url . '" class="' . $classes . '">' . $text . '</a>'
							. '<div role="tooltip" id="tip-' . (int) $apimanagerid . '-' . (int) $item->id . '">' . $tooltip . '</div>';
					}
					else
					{
